...
def save_checkpoint(state, checkpoint_path, cfg, epoch):
    model_name = 'checkpoint_epoch_'+ str(epoch) + '.pth.tar'
    file_path = osp.join(checkpoint_path, model_name)
...
def main(args):
    ...
    if hasattr(model.module, 'optimizer'): # false
    ...
    start_epoch = 0
    start_iter = 0
    if hasattr(cfg.train_cfg, 'pretrain'): # false
    ...
    for epoch in range(start_epoch, cfg.train_cfg.epoch):
        ...
        save_checkpoint(state, checkpoint_path, cfg, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='config/psenet/psenet_r50_ctw.py', help='config file path')
    parser.add_argument('--checkpoint', nargs='?', type=str, default='checkpoints')
