...
from nets.centernet_detection_onnx import CenterNet_detection_HourglassNet, CenterNet_detection_Resnet50
...
class CenterNet(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : '/root/code/AiEngineering/01-ObjectDetection/CenterNet/code/logs/ep220-loss0.988-val_loss0.931.pth',
        "classes_path"      : '/root/code/dataset/610/dataset_610_v1/dataset_610_v1/class.txt',
        #--------------------------------------------------------------------------#
        #   用于选择所使用的模型的主干
        #   resnet50, hourglass
        #--------------------------------------------------------------------------#
    ...
    def generate(self, onnx=False):
        ...
        if self.backbone == "resnet50":
            self.net = CenterNet_detection_Resnet50(num_classes=self.num_classes, pretrained=False)
        else:
            self.net = CenterNet_detection_HourglassNet({'hm': self.num_classes, 'wh': 2, 'reg':2})
    ...
    def convert_to_onnx_xian(self, onnxfile_save_path, image):
        ...
        image_data = np.repeat(image_data,[1], axis=0)
        with torch.no_grad():
            images = torch.from_numpy(np.asarray(image_data)).type(torch.FloatTensor)
            if self.cuda:
                images = images.cuda()
