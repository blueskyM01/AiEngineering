...
class COCODetection(data.Dataset):
    ...
    def __init__(self, image_path, info_file, transform=None,
        ...
        with open(info_file, 'r') as load_f:
        ...
        self.name = dataset_name # 'MS COCO'
        self.has_gt = has_gt # ture
    ...
    def pull_item(self, index):
        ...
        if self.has_gt:
            # target = [{}, {}]:获取图像中所有标注对象的信息
            # {}为：{'id': 13113, 'image_id':3494, 'category_id':1, 'segmentation':[...], 'area':12121.0, 'bbox':[x0, y0, x1, y1], 'iscrowd':0, 'attributes':{'occluded':False}}
            #       {'id': 13114, 'image_id':3494, 'category_id':1, 'segmentation':[...], 'area':721.0, 'bbox':[x0, y0, x1, y1], 'iscrowd':0, 'attributes':{'occluded':False}}
            # target = self.coco.imgToAnns[img_id]
            target = self.coco[img_name]

            # 取出target中的所有'id'，如[13113, 13114]
            # ann_ids = self.coco.getAnnIds(imgIds=img_id)

            # target = {id', 'segmentation', 'area', iscrowd', 'image_id', 'bbox', 'category_id', 'attributes'}
        ...
        if len(target) > 0:
            ...
            masks = []
            for obj in target:
                seg_t = obj['segmentation']
                seg_tnp = []
                for poly_t in seg_t:
                    poly_t_list = []
                    for i in range(len(poly_t)//2):
                        x = poly_t[i*2 + 0]
                        y = poly_t[i*2 + 1]
                        poly_t_list.append([int(x), int(y)])
                    seg_tnp.append(np.array(poly_t_list))
                mask_zero = np.zeros((height, width), dtype=np.uint8)
                mask = cv2.fillPoly(mask_zero, seg_tnp, 1)
                mask = mask.reshape(-1)
                # cv2.imwrite('result_temp/2.jpg', mask)
                masks.append(mask)
