...
class PredictionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        zpmc_bbox_layer = src.bbox_layer(bbox_x).permute(0, 2, 3, 1).contiguous()
        bbox = zpmc_bbox_layer.view(-1, int(zpmc_bbox_layer.numel() / x.size(0) / 4), 4)
        zpmc_conf_layer = src.conf_layer(conf_x).permute(0, 2, 3, 1).contiguous()
        conf = zpmc_conf_layer.view(-1, int(zpmc_conf_layer.numel() / x.size(0) / self.num_classes), self.num_classes)
        
        if cfg.eval_mask_branch:
            # mask = src.mask_layer(mask_x).permute(0, 2, 3, 1).contiguous().view(x.size(0), -1, self.mask_dim)
            zpmc_mask_layer = src.mask_layer(mask_x).permute(0, 2, 3, 1).contiguous()
            mask = zpmc_mask_layer.view(-1, int(zpmc_mask_layer.numel() / x.size(0) / self.mask_dim), self.mask_dim)
        else:
            mask = torch.zeros(int(x.size(0)), bbox.size(1), self.mask_dim, device=bbox.device)
        ...
        if cfg.eval_mask_branch:
            if cfg.mask_type == mask_type.direct:
                ...
                if cfg.mask_proto_coeff_gate:
                    ...
                    zpmc_gate_layer = src.gate_layer(x).permute(0, 2, 3, 1).contiguous()
                    gate = zpmc_gate_layer.view(-1, int(zpmc_gate_layer.numel() / x.size(0) / self.mask_dim), self.mask_dim)
...
class FPN(ScriptModuleWrapper):
    ...
    def forward(self, convouts:List[torch.Tensor]):
        ...
        lat_layer = self.lat_layers[0]
        x = x + lat_layer(convouts[2])
        out[2] = x
        
        lat_layer = self.lat_layers[1]
        _, _, h, w = convouts[1].size()
        # x = F.interpolate(x, size=(h, w), mode=self.interpolation_mode, align_corners=False)
        x = F.interpolate(x, scale_factor=(35/18, 35/18), mode=self.interpolation_mode, align_corners=False)
        x = x + lat_layer(convouts[1])
        out[1] = x

        lat_layer = self.lat_layers[2]
        _, _, h, w = convouts[0].size()
        # x = F.interpolate(x, size=(h, w), mode=self.interpolation_mode, align_corners=False)
        x = F.interpolate(x, scale_factor=(69/35, 69/35), mode=self.interpolation_mode, align_corners=False)
        x = x + lat_layer(convouts[0])
        out[0] = x
        ...
        #         fenmu_h = (h+1) / 2
        #         fenmu_w = (w+1) / 2
        #         x = F.interpolate(x, scale_factor=(float(h/fenmu_h), float(w/fenmu_w)), mode=self.interpolation_mode, align_corners=False)
...
class Yolact(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            cfg._tmp_img_h = int(img_h)
            cfg._tmp_img_w = int(img_w)
        else:
            # x = x.unsqueeze(0)
            x = x.permute(0, 3, 1, 2).contiguous()
            # x = F.interpolate(x, scale_factor=(550/720, 550/1280), mode='bilinear', align_corners=False)
            
            mean = torch.Tensor([103.94, 116.78, 123.68]).float().cuda().view(1,3,1,1)
            std  = torch.Tensor([57.38, 57.12, 58.4]).float().cuda().view(1,3,1,1)
            x = (x - mean) / std
        ...
        if self.training:
            ...
            # mask_batch = self.detect(pred_outs, self)
            # return mask_batch
            
            zpmc_loc = pred_outs['loc']
            zpmc_conf = pred_outs['conf']
            zpmc_mask = pred_outs['mask']
            zpmc_priors = pred_outs['priors']
            zpmc_proto = pred_outs['proto']

            return zpmc_loc, zpmc_conf, zpmc_mask, zpmc_priors, zpmc_proto
