...
class Detect(object):
    ...
    def __call__(self, predictions, net):
        ...
        with timer.env('Detect'):
            ...
            # mask_batch = torch.full((batch_size, 720, 1280), 0)
            ...
            for batch_idx in range(batch_size):
                ...
                #     mask_final = self.postprocess(result, score_threshold=0.15)
                # else:
                #     mask_final = torch.full((720, 1280), 0)
                
                # mask_batch[batch_idx] = mask_final
    ...
    def postprocess(self, det_output, w=720, h=1280, interpolation_mode='bilinear',
        ...
        masks = F.interpolate(masks.unsqueeze(0), scale_factor=(h/138, w/138), mode=interpolation_mode, align_corners=False).squeeze(0)

        # Binarize the masks
        masks.gt_(0.5)

        mask_final = torch.sum(masks, dim=0).transpose(1, 0)
